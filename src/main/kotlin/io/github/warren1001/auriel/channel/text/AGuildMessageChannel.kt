...
class AGuildMessageChannel {
	...
	companion object {
		private val REGEX_URL = Regex("(https?://.+?)(?: |$)")
	}
	...
	fun handleMessageReceived(event: MessageReceivedEvent): Boolean {
		...
		if (!author.hasPermission(Permission.BAN_MEMBERS)) {
			...
			if (applyWordFilters(event, message, data.wordFilters)) {
				return true
			}
			...
			// only one message
			if (data.getAsBoolean("channel:only-one-message") == true) {
	...
	fun applyWordFilters(event: MessageReceivedEvent, message: String, wordFilters: Collection<WordFilter>): Boolean {
		
		val author = event.member!!
		val channel = event.channel.asGuildMessageChannel()
		
		val filters = wordFilters.filter { it.containsMatchIn(message) }
		if (filters.isNotEmpty()) {
			var messageModified = message
			val blockedPhrases = filters.joinToString(" ") { it.name }
			event.message.delete().queue_()
			var repostId: Long? = null
			if (filters.all { it.shouldReplace() } && allowsReposting() == true) {
				messageModified = applyFilters(filters, messageModified)
				repostId = event.channel.message("${event.author.asMention} said (filtered): $messageModified").complete().idLong
			} else {
	...
	fun applyFilters(filters: List<WordFilter>, message: String): String {
		var messageModified = message
		if (REGEX_URL.containsMatchIn(messageModified)) {
			val urls = mutableListOf<String>()
			REGEX_URL.findAll(messageModified).forEach { result ->
				val url = result.groupValues[1]
				if (filters.any { it.containsMatchIn(url) }) {
					if (urls.contains(url)) {
						messageModified = messageModified.replace(url, "{filter:url:${urls.indexOf(url)}}")
					} else {
						messageModified = messageModified.replace(url, "{filter:url:${urls.size}}")
						urls.add(result.groupValues[1])
					}
				}
			}
			filters.forEach { filter -> messageModified = filter.replace(messageModified) }
			urls.forEachIndexed { index, url -> messageModified = messageModified.replace("{filter:url:$index}", "[censored link]($url)") }
		} else {
			filters.forEach { messageModified = it.replace(messageModified) }
		}
